...
};}var _proto2=_class2.prototype;_proto2.mod=function mod(gen,modid){if(!modid)modid="gen"+gen;if(modid==='gen9')return this;if(!window.BattleTeambuilderTable)return this;if(modid in this.moddedDexes){return this.moddedDexes[modid];}this.moddedDexes[modid]=new ModdedDex(gen,modid);return this.moddedDexes[modid];};_proto2.forGen=function forGen(gen){if(!gen)return this;return this.mod(gen);};_proto2.resolveAvatar=function resolveAvatar(avatar){var _window$Config;if(window.BattleAvatarNumbers&&avatar in BattleAvatarNumbers){avatar=BattleAvatarNumbers[avatar];}if(avatar.charAt(0)==='#'){return this.resourcePrefix+'sprites/trainers-custom/'+toID(avatar.substr(1))+'.png';}if(avatar.includes('.')&&((_window$Config=window.Config)!=null&&(_window$Config=_window$Config.server)!=null&&_window$Config.registered||window.Config.isReplay)){var server=Config.server||Config.defaultserver;var protocol=server.https?'https':'http';var port=server.https?server.port:server.httpport;var avatarSrc=protocol+'://'+server.host+':'+port+'/avatars/'+encodeURIComponent(avatar).replace(/\%3F/g,'?');return avatarSrc;}return this.resourcePrefix+'sprites/trainers/'+this.sanitizeName(avatar||'unknown')+'.png';};_proto2.sanitizeName=function sanitizeName(name){if(!name)return'';return(''+name).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').slice(0,50);};_proto2.prefs=function prefs(prop){var _window$Storage;return(_window$Storage=window.Storage)==null||_window$Storage.prefs==null?void 0:_window$Storage.prefs(prop);};_proto2.getShortName=function getShortName(name){var shortName=name.replace(/[^A-Za-z0-9]+$/,'');if(shortName.indexOf('(')>=0){shortName+=name.slice(shortName.length).replace(/[^\(\)]+/g,'').replace(/\(\)/g,'');}return shortName;};_proto2.getEffect=function getEffect(name){name=(name||'').trim();if(name.substr(0,5)==='item:'){return this.items.get(name.substr(5).trim());}else if(name.substr(0,8)==='ability:'){return this.abilities.get(name.substr(8).trim());}else if(name.substr(0,5)==='move:'){return this.moves.get(name.substr(5).trim());}var id=toID(name);return new PureEffect(id,name);};_proto2.getGen3Category=function getGen3Category(type){return['Fire','Water','Grass','Electric','Ice','Psychic','Dark','Dragon'].includes(type)?'Special':'Physical';};_proto2.
...
var species=this.species.get(pokemon);
...
var allowAnim=!this.prefs('noanim')&&!this.prefs('nogif');
...
var species=this.species.get(pokemon.species);
...
function ModdedDex(gen,modid){var _this2=this;this.gen=void 0;this.modid=void 0;this.cache={Moves:{},Items:{},Abilities:{},Species:{},Types:{}};this.pokeballs=null;this.
...
var _ref2=/([a-z]*)([0-9]*)/.exec(id),hpWithType=_ref2[1],hpPower=_ref2[2];
id=toID(hpWithType);
data=Object.assign({},
window.BattleMovedex[hpWithType]||{},{
basePower:Number(hpPower)||60});
...
var newBasePower=Number(id.slice(6));
...
var _newBasePower=Number(id.slice(11));
id=toID(id.substr(0,11));
data=Object.assign({},
window.BattleMovedex['frustration']||{},{
basePower:_newBasePower});
...
if(id in _table2.overrideItemDesc){
data.shortDesc=_table2.overrideItemDesc[id];
break;
}
}
...
if(id in _table6.removeType){
data.exists=false;

}else if(id in _table6.overrideTypeChart){
data=Object.assign({},data,_table6.overrideTypeChart[id]);
}
}
...
};this.gen=gen;this.modid=modid;}var _proto3=ModdedDex.prototype;_proto3.
...
var badgeFileName=getAttrib('badgefilename')||'';
var badgeName=getAttrib('badgename')||'';
var server=Config.server||Config.defaultserver;
var protocol=server.https?'https':'http';
var port=server.https?server.port:server.httpport;
var badgeSrc=protocol+'://'+server.host+':'+port+
...
var avatarFileName=getAttrib('avatarfilename')||'';
var isRequest=getAttrib('request')!==undefined;
var _server=Config.server||Config.defaultserver;
var _protocol=_server.https?'https':'http';
var _port=_server.https?_server.port:_server.httpport;
var avatarSrc=_protocol+'://'+_server.host+':'+_port+
