...
var isModdedFormatType=function(formatType){return formatType!==null&&typeof formatType!=='string';};
var getModdedFormatTableName=function(moddedFormatType){return""+moddedFormatType.mod+(moddedFormatType.isNatDex?'natdex':'only');};var
...
break;
case'ability':
var ability=this.dex.abilities.get(fId).name;
...
var modMatch=format.match(/^([a-z]+)(only|nationaldex)/);
if(modMatch){
var fullMod=modMatch[0],modName=modMatch[1],modified=modMatch[2];
format=format.slice(fullMod.length);
this.formatType={mod:modName,isNatDex:modified==='nationaldex'};
this.dex=Dex.mod(gen,getModdedFormatTableName(this.formatType));
}else if(format.includes('nationaldex')){
...
if(this.formatType==='letsgo')table=table['gen7letsgo'];
if(isModdedFormatType(this.formatType))table=table[getModdedFormatTableName(this.formatType)];
var learnset=table.learnsets[learnsetid]||BattleTeambuilderTable.learnsets[learnsetid];
...
isModdedFormatType(this.formatType)?getModdedFormatTableName(this.formatType):"gen"+
gen;
...
getBaseResults=function getBaseResults(){var _this$formatType,_this$formatType2,_this$formatType3,_this$formatType4;
...
var isDoublesOrBS=isVGCOrBS||!isModdedFormatType(this.formatType)&&((_this$formatType=this.formatType)==null?void 0:_this$formatType.includes('doubles'));
var dex=this.dex;
...
table=table['gen'+dex.gen];
}else if((_this$formatType2=this.formatType)!=null&&_this$formatType2.startsWith('bdsp')){
...
}else if((_this$formatType3=this.formatType)!=null&&_this$formatType3.startsWith('dlc1')){
if(this.formatType.includes('doubles')){
table=table['gen8dlc1doubles'];
}else{
table=table['gen8dlc1'];
}
}else if((_this$formatType4=this.formatType)!=null&&_this$formatType4.startsWith('predlc')){
if(this.formatType.includes('doubles')){
...
if(format==='cap'||format.endsWith('cap'))tierSet=tierSet.slice(0,slices.Uber).concat(tierSet.slice(slices.OU));else
if(format==='caplc'||format.endsWith('caplc'))tierSet=tierSet.slice(slices['CAP LC'],slices.Uber).concat(tierSet.slice(slices.LC));else
...
if(format==='nfe')tierSet=tierSet.slice(slices.NFE);else
if(this.formatType==='bdsp'||this.formatType==='bdspdoubles'||this.formatType==='letsgo'||this.formatType==='stadium'){
...
var species=this.dex.species.get(row[1]);for(var _i0=0;_i0<
filters.length;_i0++){var _ref7=filters[_i0];var filterType=_ref7[0];var value=_ref7[1];
switch(filterType){
case'type':
...
return results.sort(function(_ref0,_ref1){var rowType1=_ref0[0],id1=_ref0[1];var rowType2=_ref1[0],id2=_ref1[1];
...
var badAbilities=[['header',"Unviable Abilities"]];for(var _i10=0,_abilities$sort$map2=
abilities.sort().map(function(abil){return dex.abilities.get(abil);});_i10<_abilities$sort$map2.length;_i10++){var _ability=_abilities$sort$map2[_i10];
...
filters.length;_i12++){var _ref12=filters[_i12];var filterType=_ref12[0];var value=_ref12[1];
switch(filterType){
case'pokemon':
...
filters.length;_i16++){var _ref13=filters[_i16];var filterType=_ref13[0];var value=_ref13[1];
switch(filterType){
case'pokemon':
if(!Dex.hasAbility(this.dex.species.get(value),ability.name))return false;
break;
...
while(learnsetid){var _this$formatType5;
var learnset=lsetTable.learnsets[learnsetid]||BattleTeambuilderTable.learnsets[learnsetid];
if(this.formatType==='letsgo')learnset=BattleTeambuilderTable['letsgo'].learnsets[learnsetid];
if(typeof this.formatType==='string'&&(_this$formatType5=this.formatType)!=null&&_this$formatType5.startsWith('dlc1'))learnset=BattleTeambuilderTable['gen8dlc1'].learnsets[learnsetid];
if(learnset){
for(var moveid in learnset){var _BattleTeambuilderTab,_this$formatType6,_BattleTeambuilderTab2;
...
if(
this.formatType==='ssdlc1'&&(_BattleTeambuilderTab=
BattleTeambuilderTable['gen8dlc1'])!=null&&_BattleTeambuilderTab.nonstandardMoves.includes(moveid))
{
continue;
}
if(
!isModdedFormatType(this.formatType)&&(_this$formatType6=this.formatType)!=null&&_this$formatType6.includes('predlc')&&this.formatType!=='predlcnatdex'&&(_BattleTeambuilderTab2=
...
if(_move.isMax||_move.isZ||_move.isNonstandard&&(_move.isNonstandard!=='Past'||(this.formatType!=='natdex'||isModdedFormatType(this.formatType)&&!this.formatType.isNatDex)&&dex.gen===8))continue;
...
if(_move.isNonstandard==='Past'&&this.formatType!=='natdex'&&isModdedFormatType(this.formatType)&&!this.formatType.isNatDex&&dex.gen===8)continue;
moves.push(_move.id);
}
}
}
...
filters.length;_i26++){var _ref14=filters[_i26];var filterType=_ref14[0];var value=_ref14[1];
switch(filterType){
case'type':
