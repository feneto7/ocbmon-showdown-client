	...
	var AltFormPopup = this.AltFormPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			for (var i = 0; i < formCount; i++) {
				...
				buf += 'background-position:' + offset + '; background: url(' + spriteDir + '/' + (form ? baseid : toID(species.baseSpecies) + (species.forme ? '-' + toID(species.forme) : '')) + (form ? '-' + formid : '') + '.png) no-repeat; ' + spriteDim + '"';
